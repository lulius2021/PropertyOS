    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="max-h-[90vh] w-full max-w-4xl overflow-hidden rounded-2xl bg-white shadow-2xl">
        ...
        <div className="flex items-center justify-between border-b border-gray-200 px-6 py-4">
          <div>
            <h2 className="text-xl font-bold text-gray-900">Widget-Bibliothek</h2>
            <p className="mt-1 text-sm text-gray-500">Widget auswählen und zum Dashboard hinzufügen</p>
          </div>
          <button
            onClick={onClose}
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="truncate text-xs text-gray-500">{widget.description}</p>
                      </div>
                      {isActive && (
          ...
          <div className="flex-1 overflow-y-auto p-6">
            {selected ? (
              <div>
                ...
                {/* Size Selection */}
                <div className="mb-6">
                  <label className="mb-3 block text-sm font-semibold text-gray-700">Größe wählen</label>
                ...
                {/* Preview Box */}
                <div className="mb-6">
                  <label className="mb-3 block text-sm font-semibold text-gray-700">Vorschau</label>
                  <div className="rounded-lg border-2 border-dashed border-gray-300 bg-gray-50 p-8">
                    <div
                      className={`mx-auto rounded-xl border border-gray-200 bg-white p-6 shadow-sm ${
                        selectedSize === "small"
                          ? "max-w-xs"
                          : selectedSize === "medium"
                            ? "max-w-md"
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-sm font-medium text-gray-500">{selected.name}</p>
                          <p className="text-2xl font-bold text-gray-900">—</p>
                        </div>
                      </div>
                      <p className="mt-3 text-xs text-gray-500">{selected.description}</p>
                    </div>
                  </div>
                </div>
