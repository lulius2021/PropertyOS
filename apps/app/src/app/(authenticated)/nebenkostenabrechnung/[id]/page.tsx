  ...
  return (
    <div>
      ...
      <div className="mb-6 grid grid-cols-4 gap-4">
        ...
        <div className="rounded-lg border border-gray-200 bg-white p-4">
          <div className="text-sm text-gray-500">Kostenpositionen</div>
          <div className="mt-1 font-semibold">{nka.positionen?.length || 0}</div>
        </div>
        <div className="rounded-lg border border-gray-200 bg-white p-4">
          <div className="text-sm text-gray-500">Mieterpositionen</div>
      ...
      <div className="mb-4 flex gap-2">
        <button onClick={() => setActiveTab("positionen")} className={`rounded px-4 py-2 text-sm font-medium ${activeTab === "positionen" ? "bg-blue-600 text-white" : "bg-gray-200 text-gray-700 hover:bg-gray-300"}`}>
          Kostenpositionen ({nka.positionen?.length || 0})
        </button>
        <button onClick={() => setActiveTab("mieter")} className={`rounded px-4 py-2 text-sm font-medium ${activeTab === "mieter" ? "bg-blue-600 text-white" : "bg-gray-200 text-gray-700 hover:bg-gray-300"}`}>
      ...
      {activeTab === "positionen" && (
        <div className="rounded-lg border border-gray-200 bg-white shadow-sm">
          {!nka.positionen?.length ? (
            <div className="p-8 text-center text-gray-500">Noch keine Positionen. Klicken Sie auf "Berechnen".</div>
          ) : (
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  <th className="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">Kostenart</th>
                  <th className="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">Umlageschlüssel</th>
                  <th className="px-6 py-3 text-right text-xs font-medium uppercase tracking-wider text-gray-500">Betrag gesamt</th>
                  <th className="px-6 py-3 text-center text-xs font-medium uppercase tracking-wider text-gray-500">BK</th>
                  <th className="px-6 py-3 text-center text-xs font-medium uppercase tracking-wider text-gray-500">HK</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-200">
                {nka.positionen.map((p: any) => (
                  <tr key={p.id} className="hover:bg-gray-50">
                    <td className="px-6 py-4 font-medium text-gray-900">{p.kostenartBezeichnung}</td>
                    <td className="px-6 py-4 text-sm text-gray-500">{p.umlageschluessel}</td>
      ...
      {activeTab === "mieter" && (
        <div className="rounded-lg border border-gray-200 bg-white shadow-sm">
          {!nka.mieterpositionen?.length ? (
            <div className="p-8 text-center text-gray-500">Noch keine Mieterpositionen. Klicken Sie auf "Berechnen".</div>
          ) : (
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  <th className="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">Mieter</th>
                  <th className="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">Einheit</th>
                  <th className="px-6 py-3 text-right text-xs font-medium uppercase tracking-wider text-gray-500">Vorauszahlung</th>
                  <th className="px-6 py-3 text-right text-xs font-medium uppercase tracking-wider text-gray-500">Ist-Kosten</th>
                  <th className="px-6 py-3 text-right text-xs font-medium uppercase tracking-wider text-gray-500">Differenz</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-200">
                {nka.mieterpositionen.map((mp: any) => {
                  ...
                  return (
                    <tr key={mp.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 text-sm text-gray-500">{mp.mietverhaeltnis?.einheit?.einheitNr}</td>
                      ...
                      <td className={`px-6 py-4 text-right font-semibold ${diff < 0 ? "text-green-600" : diff > 0 ? "text-red-600" : "text-gray-900"}`}>
                        {diff > 0 ? "+" : ""}{diff.toFixed(2)} €
                        <div className="text-xs font-normal text-gray-500">{diff < 0 ? "Guthaben" : diff > 0 ? "Nachforderung" : "Ausgeglichen"}</div>
                      </td>
                    </tr>
                  );
                })}
